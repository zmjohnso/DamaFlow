...
export default function OnboardingScreen() {
  ...
  function handleDone(markedSkillIds: Set<number>) {
    ...
    try {
      ...
      if (markedSkillIds.size > 0) {
        ...
        bulkInsertSkillProgress(
          db,
          results.map((r) => ({ ...r, due: r.due.slice(0, 10) })),
        );
      }
  ...
  const tierMap: Record<
    Exclude<Step, "welcome" | "complete">,
    { skills: Skill[]; label: string; nextLabel: string | null }
  > = {
    beginner: {
      skills: beginnerSkills,
      label: "Beginner",
      nextLabel: "Next: Intermediate →",
    },
    intermediate: {
      skills: intermediateSkills,
      label: "Intermediate",
      nextLabel: "Next: Advanced →",
    },
